...
def main2(battle_music, sound_enabled, name):  # второй уровень
    ...
    bow = BowBar(0)
    player = Player(WIDTH // 2, HEIGHT // 2, all_sprites, 100, name, 20)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if menuning:
                    mouse_x, mouse_y = pygame.mouse.get_pos()
                    again.changes(mouse_x, mouse_y, lambda: main2(battle_music, sound_enabled, name), name,
                                  player.point)
                    home.changes(mouse_x, mouse_y, lambda: Begining(battle_music, sound_enabled), name, player.point)

        if player.counte == 300:
